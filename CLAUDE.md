...
- **课表配色**（固定亮/暗两套，无用户手选——早期的五套手选方案已移除）：色板与取色逻辑全在 `ui/screens/schedule/SchedulePalettes.kt`（`LightCourseColors`/`DarkCourseColors` 各 12 色，**同下标同色相**），`courseColor(course)` 是 @Composable，按**已应用 MaterialTheme 背景亮度**判暗自动切换（含 ThemeMode 强制与动态取色的解析结果，勿改成复刻 Theme.kt 的 ThemeMode 逻辑——会漂移）。亮色即原「经典明快」（Material 700/800，老用户浅色下零变化），暗色是同色相压暗降饱和版。两套都保证其上白色 9sp 小字对比度 ≥4.5:1——由 JVM 单测 `SchedulePaletteContrastTest` 强制（CI 跑），调色必须过它（浅色 300 系 + 白字曾低至 1.7:1）；两套长度必须一致（同测试断言），否则亮暗切换会打乱同名课程的撞色关系。`ThemePrefs.SchedulePalette` 枚举已删除，旧 prefs key `schedule_palette` 残留无害。
